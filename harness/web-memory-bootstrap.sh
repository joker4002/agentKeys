...
if should_run_step 6; then
  ...
  if [ "$active" != "true" ]; then
    ...
    # device_key_hash = the master's registered device (keccak(operator_omni)).
    # The issue-#76 K10 cap-PoP is OPTIONAL: minted without one until the master's
    # secp256k1 K10 is registered as a CAP_MINT device (heima-register-master-k10.sh)
    # — the worker accepts a no-PoP cap unless AGENTKEYS_WORKER_REQUIRE_CAP_POP=1.
