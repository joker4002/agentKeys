...
# device_key_hash = the master's registered device (keccak(operator_omni), the
# #164 passkey account). The issue-#76 K10 cap-PoP is OPTIONAL here: the master
# mints WITHOUT a PoP until its secp256k1 K10 is registered as a CAP_MINT device
# (scripts/heima-register-master-k10.sh) — the worker accepts a no-PoP cap unless
# AGENTKEYS_WORKER_REQUIRE_CAP_POP=1. Once the master K10 is registered + the
# daemon signs (BackendClient), this path carries a verified PoP.
