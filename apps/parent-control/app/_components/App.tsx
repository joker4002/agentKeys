...
import { CHAIN_PROFILE, PAIRING_STEPS } from '@/lib/demoData';
...
import type { AuditDecodeResult, MasterMemoryEntry } from '@/lib/client/types';
import type {
  Actor,
  AuditEvent,
  Namespace,
...
export function App() {
  ...
  const [pairingCeremony, setPairingCeremony] = useState<PairingRequest | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (r.ok && r.data.identity === 'verified') {
        ...
        try {
          on = localStorage.getItem('ak_onboarded') === '1';
        } catch {}
      }
    ...
    const stop = client.streamAudit(
      (e) => {
        ...
        setTimeout(
          () =>
            setEvents((prev) =>
              prev.map((x) => (x.id === e.id ? { ...x, _isNew: false } : x))
            ),
          1500
        );
      },
  ...
  const logout = () => {
    ...
    try {
      localStorage.removeItem('ak_onboarded');
    } catch {}
    setOnboarded(false);
    setIdentity(null);
    setActors([]);
  ...
  const plantDone = async () => {
    ...
    if (r.ok) {
      ...
      showToast(
        `Prepared memory planted · ${r.data.planted} new, ${r.data.skipped} deduped.`
      );
    } else {
  ...
  const finishPairingCeremony = async () => {
    ...
    if (a.ok) setActors(a.data);
    showToast(
      `${req.agent} paired · cap-tokens minted · session key handed off.`
    );
    go('pairing');
  };
  ...
  const sectionAttr = (
    ['audit', 'memory', 'pairing', 'chain', 'logo'] as string[]
  ).includes(page)
    ? page
    : undefined;
  ...
  if (!onboarded) {
    return (
      <OnboardingScreen
        onComplete={() => {
          try {
            localStorage.setItem('ak_onboarded', '1');
          } catch {}
          setOnboarded(true);
          go('actors');
        }}
  ...
  const whoLabel =
    master?.label ??
    (identity?.email ? maskEmail(identity.email, maskEm) : undefined) ??
    'O_master';
  ...
  return (
    <div className="app">
          ...
          <button
            ...
            title={
              pairingRequests.length
                ? `${pairingRequests.length} pairing request`
                : 'no pending requests'
            }
          >
            ◉
            {pairingRequests.length > 0 && (
              <span className="badge">{pairingRequests.length}</span>
            )}
          </button>
          <button
            className="btn sm"
            ...
            title={
              maskEm
                ? 'Email masked (for screen-sharing) — click to show'
                : 'Email shown — click to mask'
            }
          >
            {maskEm ? '🕶' : '👁'}
          </button>
          <span className="who" title={whoOmni ?? ''}>
            <span className="who-text">
              {whoOmni ? `${whoLabel} · ${shortOmni(whoOmni)}` : whoLabel}
            </span>
          </span>
          <button
            className="btn sm"
            onClick={logout}
            title="Clear this session and return to login"
          >
            log out
          </button>
        </div>
      ...
      <aside className={`app-side ${sideOpen ? 'open' : ''}`}>
        ...
        <button
          className={`nav-item ${page === 'actors' ? 'active' : ''}`}
          onClick={() => go('actors')}
        >
        ...
        <button
          ...
          onClick={() => go('memory')}
        >
          <span className="marker">[◇]</span> memory
        ...
        <button
          className={`nav-item ${page === 'chain' ? 'active' : ''}`}
          onClick={() => go('chain')}
        >
        ...
        <button
          className={`nav-item ${page === 'logo' ? 'active' : ''}`}
          onClick={() => go('logo')}
        >
        ...
        {actors.map((a) => (
          <button
            key={a.id}
            className={`nav-item ${
              page === 'detail' && actorId === a.id ? 'active' : ''
            }`}
      ...
      {memoryView && (
        <Modal
          ...
          footer={
            <button className="btn" onClick={() => setMemoryView(null)}>
              close
            </button>
          }
        >
          <dl className="kvs" style={{ marginBottom: 14 }}>
            ...
            <dt>envelope</dt>
            <dd className="mono">{memoryView.version} · AES-256-GCM · k3 v1</dd>
            <dt>bytes</dt>
            <dd className="mono">{memoryView.bytes}</dd>
            <dt>updated</dt>
            <dd>{memoryView.updated}</dd>
          </dl>
          <div
            style={{
              fontSize: 10,
          ...
          >
            ...
            <div className="modal-body">
              ...
              >
                Binding{' '}
                <span className="mono">
                  O_master{pairingCeremony.derivation}
                </span>{' '}
                under your master identity. Each on-chain step is a real Heima
                transaction.
              </div>
              <CeremonyRunner
                steps={PAIRING_STEPS}
                onDone={finishPairingCeremony}
                stepMs={680}
              />
            </div>
          </div>
...
function EventDecodeModal({
  ...
  const client = useClient();
  const [decode, setDecode] = useState<AuditDecodeResult | null>(null);
  ...
  useEffect(() => {
    ...
    setError(null);
    setDecode(null);
    client.decodeAuditEvent(event.id).then((r) => {
      if (cancelled) return;
      if (r.ok) {
        setDecode(r.data);
      } else {
        setError(r.status.detail ?? r.status.reason);
      }
      setLoading(false);
    });
    return () => {
  ...
  const tx = decode?.tx;
  const envelope = decode?.envelope;
  const signer =
    event.actor === 'Sara (master)'
      ? 'D_pub_master_iphone'
      : 'D_pub_' + event.actor.toLowerCase().replace(/[^a-z]/g, '');
  const txHref = tx?.hash
    ? `${CHAIN_PROFILE.explorer}/tx/${tx.hash}`
    : undefined;
  return (
    <Modal
    ...
    >
      <dl className="kvs">
        ...
        <dd>{event.detail}</dd>
        <dt>worker</dt>
        <dd className="mono">{event.chip}-service</dd>
        <dt>decode</dt>
        <dd>
          {loading
            ? 'loading from daemon'
      ...
      <div className="hr-ascii" style={{ margin: '14px 0' }}>
        {'─'.repeat(220)}
      </div>
      ...
      <div className="tx-decode">
        {loading && (
          <div className="tx-row">
            ...
            <span className="tx-v">decoding…</span>
          </div>
        )}
        {error && (
          <div className="tx-row">
            <span className="tx-k">status</span>
            <span className="tx-v">{error}</span>
          </div>
        )}
        {!loading && !error && !tx && (
          <div className="tx-row">
            <span className="tx-k">status</span>
            <span className="tx-v">no tx calldata stored for this row</span>
          </div>
        )}
        {tx && (
          <>
            <div className="tx-row">
              <span className="tx-k">tx_hash</span>
              <span className="tx-v mono">{tx.hash ?? 'not anchored yet'}</span>
            </div>
            <div className="tx-row">
              <span className="tx-k">to</span>
              <span className="tx-v mono">
                {tx.contract}
                {tx.to ? ` · ${tx.to}` : ''}
              </span>
            </div>
            <div className="tx-row">
              <span className="tx-k">selector</span>
              <span className="tx-v mono">{tx.selector}</span>
            </div>
            <div className="tx-row">
              <span className="tx-k">function</span>
              <span className="tx-v mono">{tx.fn}</span>
            </div>
            {tx.args.map((arg) => (
              <div className="tx-row" key={`${arg.name}-${arg.kind}`}>
                <span className="tx-k">
                  {arg.name}
                  <span className="mono">:{arg.kind}</span>
                </span>
                <span className="tx-v mono">
                  {formatDecodedValue(arg.value)}
                </span>
              </div>
            ))}
          </>
      ...
      {envelope && (
        <>
          ...
          >
            decoded audit envelope
          </div>
          <div className="tx-decode">
            <div className="tx-row">
              <span className="tx-k">op</span>
              <span className="tx-v mono">
                {envelope.op} · {envelope.opKind}
              </span>
            </div>
            <div className="tx-row">
              <span className="tx-k">result</span>
              <span className="tx-v">{envelope.result}</span>
            </div>
            <div className="tx-row">
              <span className="tx-k">actor_omni</span>
              <span className="tx-v mono">{envelope.actorOmni}</span>
            </div>
            <div className="tx-row">
              <span className="tx-k">operator_omni</span>
              <span className="tx-v mono">{envelope.operatorOmni}</span>
            </div>
            <div className="tx-row">
              <span className="tx-k">envelope_hash</span>
              <span className="tx-v mono">{envelope.envelopeHash}</span>
            </div>
            {envelope.intentText && (
              <div className="tx-row">
                <span className="tx-k">intent</span>
                <span className="tx-v">{envelope.intentText}</span>
              </div>
            )}
            {envelope.intentCommitment && (
              <div className="tx-row">
                <span className="tx-k">intent_commitment</span>
                <span className="tx-v mono">{envelope.intentCommitment}</span>
              </div>
            )}
            <div className="tx-row">
              <span className="tx-k">op_body</span>
              <span className="tx-v mono">
                {formatDecodedValue(envelope.opBody)}
              </span>
            </div>
          </div>
        </>
      ...
      <div className="muted" style={{ fontSize: 10.5, marginTop: 10 }}>
        decoded by ui-bridge · {CHAIN_PROFILE.display}
      </div>
    </Modal>
  );
}
...
function ChainPage() {
  ...
  const [picked, setPicked] = useState<(typeof p.contracts)[number] | null>(
    null
  );
  return (
    <>
      <div className="page-head">
        <div>
          ...
          <h1>
            <span className="muted serif">/</span> chain
          </h1>
          <div className="desc">
            Four stage-1 contracts deployed via Foundry. Tier-2 audit anchors a
            Merkle root here every 2 minutes.
          </div>
        </div>
      </div>
      <div className="stats">
        <div className="stat">
          ...
          <div className="k">AGENTKEYS_CHAIN</div>
        </div>
        <div className="stat">
          <div className="v">{p.chainId}</div>
          <div className="k">chain id</div>
        </div>
        <div className="stat">
          <div className="v">{p.block}</div>
          <div className="k">latest block</div>
        </div>
        <div className="stat">
          <div className="v">{p.contracts.length}</div>
          <div className="k">contracts deployed</div>
        </div>
      </div>
      <div className="panel">
        <div className="panel-head">
          <span>── deployed contracts · stage-1</span>
        </div>
              ...
              {p.contracts.map((c) => (
                ...
                >
                  <td>
                    ...
                    <div className="secondary">{c.purpose}</div>
                  </td>
                  ...
                  <td className="right">
                    <a
                      href={`${p.explorer}/address/${c.addr}`}
                      target="_blank"
                      rel="noreferrer"
                      style={{ fontSize: 11 }}
                    >
      ...
      {picked && (
        <Modal
          ...
          footer={
            <a
              className="btn primary"
              href={`${p.explorer}/address/${picked.addr}`}
              target="_blank"
              rel="noreferrer"
            >
              view on {p.name} explorer ↗
            </a>
          }
        >
          <dl className="kvs">
            ...
            <dt>deployed at</dt>
            <dd>{picked.deployedAt}</dd>
            <dt>purpose</dt>
            <dd>{picked.purpose}</dd>
            <dt>verify</dt>
            <dd className="mono" style={{ fontSize: 11 }}>
