...
export function selectBackend(): AgentKeysClient {
  ...
  const kind = (process.env.NEXT_PUBLIC_AGENTKEYS_BACKEND ??
    'daemon') as BackendKind;
  if (kind === 'core') {
    const loadCore = eval('require') as (id: string) => {
      CoreBackend: new (brokerUrl: string) => AgentKeysClient;
    };
    const { CoreBackend } = loadCore('./core');
    // Phone-first host: the WASM core talks to the broker directly (no daemon). X1.
    return new CoreBackend(
      process.env.NEXT_PUBLIC_AGENTKEYS_BROKER_URL ??
