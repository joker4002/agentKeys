...
**Status:** plan (2026-06); **item 1A landed** (#207, the default-preset bootstrap below). The **product/onboarding view** of the classifier design — how a real user's `Config` (taxonomy + policy) is *bootstrapped*, and how an agent's scopes + memory are *auto-distributed* at connect time. The classifier internals (COMPILE/TAG, the determinism guardrail, the catalog, the flywheel, the three phases) live in [`../classifier-service.md`](../classifier-service.md) (#178) and are **not** re-specified here. The encrypted, master-only `Config` substrate this writes to is **landed** (#201 Phases 0–5, [`config-data-class-memory-list.md`](config-data-class-memory-list.md)). Promote to `spec/` once the first production COMPILE ships.

> **Implementation status (#207).**
> - **1A — default-preset bootstrap: LANDED.** The daemon ships ~10 bundled role presets ([`crates/agentkeys-daemon/src/presets.rs`](../../../crates/agentkeys-daemon/src/presets.rs); default = the rich adult-household profile) behind `GET /v1/master/config/presets` + `POST /v1/master/config/init`, which **authors** `config/memory-taxonomy.enc` (master-self Config write — the category index, NOT scope grants, so no K11) as a read-modify-write **merge** so a later test-only `plant` can never clobber it; `apps/parent-control` renders the two-entry-point setup screen (A live, B placeholder).
> - **Items 2 + 3 + 6 — classifier substrate: LANDED.** `CapOp::Classify` + `/v1/cap/classify` (broker `cap.rs` + worker `verify.rs`, the 2-crate enum add, data-class-bound); the `agentkeys-worker-classify` crate (a COMPUTE gate — COMPILE + TAG, same cap+chain-verify chain, NO S3, port 9097); and the bundled **category catalog** (`catalog.rs` — entity→category + per-category **sensitivity floor** + **signed vendor overlays** bounded by the floor). Deploy-wired into `setup-broker-host.sh` + `dns-upsert-workers.sh` + the env files (prod/test/CI). The classify engine is the deterministic tier-0 (catalog + keyword rules); the LLM engine is the deferred enhancement.
> - **1B — NL → COMPILE UI: DEFERRED** (the worker's `/v1/classify/compile` exists; the onboarding UI is deferred).
> - **Items 5 + 7 + 8 — classify bridge + auto-distribution: LANDED (daemon + parent-control).** The daemon **classify bridge** (`--classify-url` → mint a master-self `Classify` cap → worker `/v1/classify/tag`, with a local `agentkeys-catalog` tier-0 fallback) backs `POST /v1/master/classify/tag` (**item 7**, cred auto-categorize) and `POST /v1/master/classify/propose` (**item 5**, connect-time auto-distribute — classify the agent's surface, propose scopes tiered by the catalog sensitivity: `auto` for Safe, `k11` for Sensitive). The agent's surface unifies **item 8** (the master's memory namespaces it can inherit, classified by category floor — sensitive namespaces land in the explicit-pick tier) + **item 7** (its cred services). `apps/parent-control`'s actor detail renders the `AutoDistributePanel` (propose → safe-set auto-confirm / sensitive per-grant Touch ID). **Load-bearing invariant (§3.2) holds by construction:** `propose` writes NO scope — only the K11-gated grant path does — so an unconfirmed sensitive category never becomes a grant.
>   - *Caveat:* the on-chain `setScope` write for a confirmed cred-service grant rides the existing scope-mutation path; the panel currently surfaces the confirm gesture + tiering (the security-relevant UX) and queues the cap-mint. Wiring confirm → a live per-service `setScope` is the remaining follow-up.
...
**Sequencing:** the taxonomy *storage* is done; what changes is the **source** (today: derived from `plant`; target: authored by COMPILE / default-preset). Ship order: (i) **✅ default-preset bootstrap (1A) — LANDED**, writing a real authored taxonomy → (ii) classifier-service + `CapOp::Classify` → (iii) NL-COMPILE (1B, **deferred**) → (iv) connect-time auto-distribute (3) → (v) catalog + vendor overlays (R2) + cred categorize (4) → (vi) telemetry priors (R4) + inheritance (R3). Each is independently shippable (#178 §1).
