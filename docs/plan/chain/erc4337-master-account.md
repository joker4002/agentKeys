...
| # | Phase | What | Gate |
|---|---|---|---|
| **E0** | Threat-model delta | The `userOpHash`-is-full-intent argument; ED-aware funding security; bundler trust model (unsafe-mode private mempool); what Solution A deletes and why it's safe → [`erc4337-threat-model.md`](erc4337-threat-model.md) ✅ drafted. | review sign-off |
...
- **E7** (ceremony binding) — **CLI hardware register WIRED; browser frontend + coordinated cutover remain.** Both UserOp signers are built in the Rust CLI (`crates/agentkeys-cli/src/k11_webauthn.rs`, **no python**): the **hardware** `k11 webauthn-{keygen,userop-sign}` (the operator's Touch ID / Secure-Enclave K11 signs the `userOpHash` via a localhost WebAuthn ceremony; **challenge == userOpHash**, no sha256 wrap — derives the account from the real `cose_pubkey_hex`) and the **software** `k11 software-{keygen,sign}` (file key, CI-only; the deprecated `harness/scripts/erc4337-webauthn-sign.py` is the byte-spec it ports). `register_first_master` / `erc4337-register-master.sh --signer` use **HARDWARE locally, SOFTWARE under `--ci`** (the EOA register is deprecated). The software path is **validated `true` against the live mainnet K11Verifier** (zero gas); the **hardware ceremony is operator-verified** — it can't run headlessly. Remaining: (a) wire the **browser** Touch ID to the UserOp in `ceremony.tsx` (the daemon web-flow frontend — still display-only); (b) the registry **cutover** (account-as-`msg.sender` for the E3/E7 account-auth redeploy) — note the software register already exercises account-as-`msg.sender`, so the register itself is not cutover-blocked.
- **E8** (mechanism proof) ✅ **ran green on Heima mainnet** — `harness/erc4337-master-e8.sh`: a passkey-only flow (no secp256k1 key, **throwaway software passkey**) deployed an account via the factory + landed a real master mutation (`addSigner`) via a WebAuthn-signed UserOp through `handleOps` (acct `0x3e79925F41E46CA87DD1103a572af7449CCd25a9`, `activeSignerCount 1→2`; reuse-mode also verified on `0x417F…`). **Unhooked from `phase1-wire-demo.sh`** — it is a standalone *mechanism smoke*, NOT the ceremony (that is E7 above). The real passkey-only-master e2e acceptance (onboarding-bound, fresh per run) lands with E7 + the cutover.
