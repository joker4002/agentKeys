...
| K11 WebAuthn enrollment in CLI | ✅ **shipped** stage-1 stub — `agentkeys k11 enroll/assert` CLI subcommands + library helpers in `crates/agentkeys-cli/src/k11.rs`. Stub mode produces deterministic bytes that satisfy the on-chain `k11Assertion.length != 0` gate; `AGENTKEYS_K11_STUB=0` errors with pointer to stage-2 (#90 webauthn-rs integration). | Real WebAuthn via `webauthn-rs` + Touch ID/Face ID lands in stage 2 |
