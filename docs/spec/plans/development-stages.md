...
Both phases shipped — see Shipped table above. Scratch notes: [`../../stage7-wip.md`](../../stage7-wip.md).

**Operational follow-ups (not architectural blockers):**

- Public TLS hosting of `$BROKER_OIDC_ISSUER` so `aws iam create-open-id-connect-provider` can fetch the JWKS. Per-operator deployment task; recipe in [`cloud-bootstrap.md` §4 "OIDC federation"](../../cloud-bootstrap.md).
