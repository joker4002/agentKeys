...
- **Don't `git pull` by hand on the broker** — `--ref` does the fetch/checkout/pull. If a prior `sudo bash …` run left repo files **root-owned**, a manual `git pull` (as `agentkey`) dies with `error: unable to unlink old '…': Permission denied`. Just run `setup-broker-host.sh --ref …` (its git runs as root and replaces them); to restore manual git, run `sudo chown -R agentkey:agentkey ~/agentKeys` once. The broker uses plain **git**, not jj — jj is only for the laptop→origin push.
