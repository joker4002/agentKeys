...
| [`agent-iam-strategy.md`](./agent-iam-strategy.md) | **★ Strategic anchor — source of truth for positioning, scope, and roadmap.** AgentKeys is the Agent IAM and memory control plane for the AI device era. Three-layer positioning (AI Device Account / Agent IAM / Trust Substrate), three audiences (consumer / B2B / regulator). Captures four architecture corrections from ChatGPT critique: bounded revocation (immediate online, TTL-bounded offline), two-tier audit (real-time off-chain + batched on-chain anchor), delegation as preview-only in v1, dual narrative not single. Phase 1 = three-act IAM demo (permissioned memory + deterministic denial + online revocation). Future planning references this doc. | Strategic anchor |
