...
| Step 7 injects the *whole* namespace, unranked | hook fell back (no query extracted, or `OPENVIKING_ENDPOINT` not baked) | confirm Step 6 baked the env. The hook reads the turn from the host payload — top-level `query`/`prompt`/`messages` AND the documented `extra` envelope + structured `content` blocks. Validate extraction against the REAL runtime shape with `hermes hooks test pre_llm_call` (the synthetic `printf '{"query":…}'` does not exercise the real envelope) |
...
| `pre_llm_call` hook slow / oversized when OpenViking is degraded | the WHOLE OpenViking ranking — find + every `content/read`, **shared across all namespaces in the turn** — is bounded by one **overall deadline** (auto-clamped below the host hook timeout wire.rs bakes, `timeout: 5`s), plus a per-request timeout, a read-fan-out cap, and a per-response **byte cap**, so a slow / stalled / oversized server falls back to the lexical engine **in time**, never hanging or OOMing the turn | tune `OPENVIKING_RANK_DEADLINE_MS` (default 3000, shared per-turn budget — clamped to host_timeout − 1s), `OPENVIKING_TIMEOUT_MS` (default 2000), `OPENVIKING_MAX_URI_READS` (default 64 — a safety bound; reads stop early once the output budget is filled), `OPENVIKING_MAX_RESPONSE_BYTES` (default 1 MiB) |
