...
// (`Option<u64>` here vs a required `u64` there). `service` is still the
// namespace-qualified signed service `memory:<ns>` (arch.md §896) — build it
// with `memoryService(ns)`, never a bare `memory` (→ `service_not_in_scope`).
// The cap-token *response* shape stays local for now (a follow-up unifies it
// with the native client's opaque token).
