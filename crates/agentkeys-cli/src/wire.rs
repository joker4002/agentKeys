...
impl HermesAdapter {
    ...
    fn managed_block_for_dir(&self, dir: &std::path::Path) -> String {
    ...
    fn managed_block(&self, _req: &WireRequest) -> Result<String> {
        Ok(self.managed_block_for_dir(&self.hooks_dir()?))
    }
}
...
fn merge_block(path: &std::path::Path, block: &str, check_only: bool) -> Result<Outcome> {
    ...
    if existing
        .lines()
        .any(|l| l.trim_start() == l && l.starts_with("hooks:"))
    {
...
mod tests {
    ...
    fn managed_block_is_valid_shape() {
        let a = HermesAdapter;
        let dir = std::path::Path::new("/tmp/agentkeys-wire-test-home/.hermes/agent-hooks");
        let block = a.managed_block_for_dir(dir);
