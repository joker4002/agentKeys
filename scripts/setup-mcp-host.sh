...
if [ "$WITH_NGINX" = "yes" ]; then
  ...
  want_vhost=$(cat <<EOF
# Generated by scripts/setup-mcp-host.sh — DO NOT HAND-EDIT
# Terminates TLS for ${DOMAIN} and upgrades wss → ws to the local
# mcp-endpoint-server on 127.0.0.1:${RELAY_PORT}.
...
server {
  ...
  # 智控台 health probe (HTTP, not WS) — same upstream, no upgrade.
  ...
  # Direct healthz for the MCP server when running --transport=http on a
  # secondary listener; harmless if it 404s.
  location = /healthz {
...
EOF
  ...
  if [ "$want_vhost" = "$got" ]; then
    skip "${NGINX_SITE##*/} already up to date"
  else
