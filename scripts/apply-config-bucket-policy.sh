#!/usr/bin/env bash
# scripts/apply-config-bucket-policy.sh — apply the v3 PrincipalTag
# policy to $CONFIG_BUCKET (the config-only bucket, per arch.md §17.2) (#201).
#
# Mirror of scripts/apply-memory-bucket-policy.sh, scoped to config.
#
# Idempotent: re-running is a no-op once the v3 markers
# (ConfigListV3 + ConfigObjectsV3) are present.
#
# Required env: ACCOUNT_ID, REGION, CONFIG_BUCKET
...
CONFIG_ROLE_ARN="${CONFIG_ROLE_ARN:-arn:aws:iam::${ACCOUNT_ID}:role/agentkeys-config-role}"
...
# Read current
log "Reading current bucket policy on s3://$CONFIG_BUCKET"
current_policy=$(aws s3api get-bucket-policy \
                   --bucket "$CONFIG_BUCKET" --region "$REGION" \
...
if [ -n "$current_policy" ]; then
  has_v3_sid=$(echo "$current_policy" \
    | jq '[.Statement[] | select(.Sid == "ConfigListV3" or .Sid == "ConfigObjectsV3")] | length' 2>/dev/null || echo 0)
  if [ "${has_v3_sid:-0}" -gt 1 ]; then already_v3=1; fi
fi
if [ "$already_v3" = "1" ]; then
  skip "policy already has v3 markers (ConfigListV3 + ConfigObjectsV3)"
  exit 0
fi
...
if [ -n "$current_policy" ]; then
  backup="/tmp/config-bucket-policy-backup-${CONFIG_BUCKET}-${ts}.json"
...
# v3 memory-bucket policy.
new_policy=$(jq -n \
  --arg bucket "$CONFIG_BUCKET" \
  --arg role_arn "$CONFIG_ROLE_ARN" '{
    Version: "2012-10-17",
    Statement: [
      {
        Sid: "ConfigListV3",
...
log "Applying v3 config-bucket policy"
aws s3api put-bucket-policy --bucket "$CONFIG_BUCKET" --region "$REGION" \
  --policy "$new_policy" \
  || die "put-bucket-policy failed"

log "Confirming write"
applied=$(aws s3api get-bucket-policy --bucket "$CONFIG_BUCKET" --region "$REGION" \
            --query Policy --output text 2>&1)
sid_count=$(echo "$applied" | jq '[.Statement[].Sid] | length')
ok "policy applied; $sid_count statement(s) live"

ok "config-bucket policy applied"
