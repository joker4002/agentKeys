...
**Exempt (NOT deploy entry points):** `setup-dev-env.sh` (developer-workstation bootstrap — rustup/node/jj, not a deploy), `*-demo.sh` / `mcp-demo-mode-*.sh` demos, read-only `verify-*.sh` checks, `install-agentkeys-cli.sh`, `lint-wiki.sh`, and surgical `heima-*-revoke`/`-rotate` helpers — tools, run on their own.
