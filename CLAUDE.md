...
|---|---|---|---|
| **1. Broker cap-mint** | The session JWT's `agentkeys.omni_account` claim MUST match the request's `operator_omni`. Also: `device.operator_omni == session_omni`, `device.actor_omni == req.actor_omni`, `device.roles & ROLE_CAP_MINT`, `isServiceInScope(operator, actor, service) == true`, **and a valid K10 cap proof-of-possession** (`verify_cap_pop`: `client_sig` recovers to an address whose `keccak == device_key_hash`, fresh — issue #76). Returns `OperatorMismatch` / `DeviceBindingMismatch` / `DeviceRoleMissing` / `ServiceNotInScope` / `CapPopInvalid` otherwise. | [`handlers/cap.rs`](crates/agentkeys-broker-server/src/handlers/cap.rs) — `mint_cap()` + `verify_cap_pop()` | [`harness/v2-stage3-demo.sh`](harness/v2-stage3-demo.sh) step 13 (NEGATIVE cap-mint with cross-actor `operator_omni` → HTTP 4xx) + the NEGATIVE stripped-`client_sig` step (#76) |
| **2. Worker chain-verify** | Independent re-check of layer-1 invariants from the worker's perspective — defense-in-depth against broker compromise. `verify_signature` (broker cap-sig), **`check_client_pop` (K10 proof-of-possession, re-verified independently of the broker — a cap without a valid `client_sig` is REJECTED, so a compromised broker cannot mint a usable cap; issue #76, gated by `AGENTKEYS_WORKER_REQUIRE_CAP_POP`, default enforce)**, `check_chain_device`, `check_chain_scope`, `check_chain_k3_epoch`. | [`crates/agentkeys-worker-creds/src/verify.rs`](crates/agentkeys-worker-creds/src/verify.rs) (shared by the cred/memory/config workers) | [`harness/v2-stage3-demo.sh`](harness/v2-stage3-demo.sh) steps 11+12 (full HTTP roundtrip exercises every verify hook) |
