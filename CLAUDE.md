...
**The broker/worker client protocol — cap-mint (the four `/v1/cap/*` endpoints), the STS relay, worker `/v1/memory/{put,get}`, audit append, the `memory:<ns>` service builder, and the `0x`-omni normalizer — is owned by ONE crate, [`agentkeys-backend-client`](crates/agentkeys-backend-client/) (request/response field types co-owned with [`agentkeys-types`](crates/agentkeys-types/)). Never re-type a cap/worker body in a second Rust path or in bash.**
...
- **Rust callers share the crate.** The MCP server's [`HttpBackend`](crates/agentkeys-mcp-server/src/backend/http_backend.rs) is a thin delegate over `BackendClient`; the daemon's [`ui_bridge`](crates/agentkeys-daemon/src/ui_bridge.rs) real-memory path calls it directly. A drifted shape is a **compile error**.
